const encode = (input) =>
    [...input]
        .map((x, i) => [x.charCodeAt(0), i]) // [код символа, индекс] массив массивов
        .sort() // отсортировать по кодам, по индексу найти соответствующее место
        .flatMap((x) => x) // теперь одномерный массив
        .join('.') // между кодами и индексами стоят точки - строка
        .match(/./g) // массив код точка индекс
        .flatMap((x, i) => new Array(x == '.' ? 1 : 2 + x * 2).fill((1 + i) % 2)) // (. - 1, код/индекс - 2+х*2) - длинна 0 или 1
        .join('') // строка 0 и 1
        .replace(/(([01])\2*)/g, (x) => `${+x ? '.' : '-'}${x.length}`); // . или - и длинна 0 или 1

const decode = (input) =>
    input
        .replace(/.?-?\d*/g, (x) => new Array(+x.slice(1)).fill(x[0] === '.' ? 1 : 0).join(''))
        .replace(/(([01])\2*)/g, (x) => (x.length === 1 ? '.' : (x.length - 2) / 2))
        .split('.')
        .reduce((acc, el, i, arr) => {
            if ((i + 1) % 2) acc.push([String.fromCharCode(el), arr[i + 1]]);
            return acc;
        }, [])
        .sort((a, b) => a[1] - b[1])
        .map((el) => el[0])
...
console.log(encode(decode(test)) === test);
